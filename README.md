 ...
 * `(dm-names:)`, `(dm-values:)` and `(dm-entries:)` have been renamed to `(dm-names:)`, `(dm-values:)` and `(dm-entries:)`, to better convey that these operate exclusively on datamaps. The original names remain as aliases.
 ...
 * Added `(input:)` and `(force-input:)`, variations of `(input-box:)` and `(force-input-box:)` which produce single-line `<input>` elements instead of `<textarea>` elements. Instead of a height number, these take a string that specifies the HTML "placeholder" attribute for the input element.
