 ...
 * Due to the above alteration, the link changers `(link:)`, `(link-reveal:)` and `(link-repeat:)` now accept an optional changer as their second value. This is used to style just the link, leaving the revealed hook unchanged.
